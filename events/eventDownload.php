...
if ($result->num_rows > 0) {
    ...
    echo "<tr><th>ID</th><th>Title</th><th>Description</th><th>Download</th></tr>";
    // output data of each row
    while($row = $result->fetch_assoc()) {
         $imageURL = 'http://localhost/gulbir/admindashboard/uploadCM/'.$row["dfile_name"];
       
        echo "<tr><td>" . $row["dID"]. "</td>
        <td>" . $row["title"]. "</td> 
        <td>" . $row["description"]. "</td> 
         <td>  <a id='" .$row["dID"]. "' href='".$imageURL."' download>Download</a></td></tr>";
    }
    echo "</table>";
} else {
    echo "0 results";
